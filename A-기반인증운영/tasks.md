...
Playwright로 헤드리스 브라우저를 직접 띄워 아래 항목을 하나씩 클릭·확인했다 (`npm run dev` + 실제 폼 입력·클릭, 스크린샷으로 결과 확인).

- [x] `npm run dev` → 첫 화면에 "시뮬레이션 모드 · 사용자 20명 · 태그 20개 · 밸런스게임 질문 30개"가 뜬다
- [x] `test@gmail.com`으로 로그인 시도 → "경기대학교 이메일(@kyonggi.ac.kr)만 사용할 수 있습니다"
- [x] 본인 학교 메일 형식(`아무거나@kyonggi.ac.kr`)으로 로그인 → `/onboarding`으로 이동
- [x] 태그를 5개 고른 뒤 6개째 클릭 → "최대 5개까지 고를 수 있어요"가 뜨고 선택되지 않는다
- [x] 프로필 작성 완료 → `/search`로 이동 시도 확인 (C의 Phase 3이 아직 없어 404 — 예상된 정상 동작)
- [x] 같은 이메일로 다시 로그인 → 온보딩을 건너뛴다
- [x] 개발자 도구 → Application → Local Storage에서 `friend-app-sim-v1` 키 안에 내 프로필이 생긴 것을 확인
- [x] 콘솔(자동화 스크립트)에서 store를 불러와 계정을 `is_admin = true`로 바꿔 저장
- [x] 관리자 계정으로 `/admin` 접속 → 지표 카드 6개가 보인다
- [x] 일반 계정으로 `/admin` 접속 → `/search`로 튕긴다
- [x] 💾 `chore: A 트랙 화면 확인 완료`

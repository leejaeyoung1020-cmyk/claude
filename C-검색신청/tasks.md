...
- [x] `app/requests/page.tsx` — 탭 2개 (받은 신청 / 보낸 신청)
- [x] 받은 신청: 인사말 + 프로필 요약 + 수락/거절 버튼
- [x] 보낸 신청: 상대 프로필 + 상태. **거절과 무응답을 똑같이 "대기 중"으로 표시** 🔍 (SPEC 4.4)
- [x] `lib/mock/api.ts`의 `respondFriendRequest(requestId, accept)`를 화면에서 직접 부른다
- [x] 수락 성공 시 반환된 대화방 id로 `/chat/[id]` 이동
- [x] 🔍 다른 탭에서 두 번째 계정으로 로그인 → `/requests`에 신청이 보인다 (Playwright로 같은 컨텍스트 안에 탭 2개를 열어 확인 — u1→u2 시드 신청이 u2 화면에 보임)
- [x] 🔍 수락 → `/chat/[id]`로 이동을 시도한다 — **D의 채팅 화면이 아직 없어 404 (A가 Phase 1에서 확인한 것과 같은 이유로 예상된 정상 동작)**. `respondFriendRequest()`가 대화방을 실제로 만드는 것은 `mockApi.test.ts`로 이미 확인됨
- [x] 🔍 거절 → 보낸 쪽 화면에 "거절당했다"는 표시가 **없다** (Playwright로 확인 — "대기 중"만 보이고 "거절"이라는 단어 자체가 화면에 없음)
- [x] 🔍 받은 신청 배지(개수)가 상단 네비게이션에 뜬다 (Playwright로 확인 — "신청함 (1)")
- [x] B에게 알림 — `app/profile/[id]/page.tsx` 완성, 차단·신고 버튼 붙여도 됨
- [x] D에게 알림 — 수락 시 대화방이 생기니 실제 흐름으로 채팅 테스트 가능
- [x] 💾 `feat: 받은/보낸 신청 목록 · 수락 · 거절`
